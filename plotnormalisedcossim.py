...
compassnns = ['voetbal', 'keuken', 'lachen', 'pannenkoeken', 'gemeente']
...
for year in range(2010, 2019):
    for part in range(1, 3):
        ...
        for word in compassnns:
            print(w2v.wv.similarity('kanker', word)-mean/sd)
            if word not in data.keys():
                data[word] = [(w2v.wv.similarity('kanker', word)-mean)/sd]
            else:
                data[word].append((w2v.wv.similarity('kanker', word)-mean)/sd)
        #except:
        #    print(yearpart)
        #    for word in compassnns:
        #        if word not in data.keys():
